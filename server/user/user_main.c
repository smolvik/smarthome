...
static void ICACHE_FLASH_ATTR connected_cb(void *arg) 
{
  ...
  //os_sprintf(transmission, "{\"temperature\":\"%d\",\"gas\":\"%d\"}", valTemperature, 15);				
  os_sprintf(transmission, "%d", valTemperature);
...
{
        uint8_t buffer[128];
        uint32_t t[] = {25,15,40,18};
        uint32_t g = 15;
        static int idx = 0;
    
        os_sprintf(buffer, "{\"temperature\":\"%d\",\"gas\":\"%d\"}", t[idx], g); idx = (idx+1)&0x03;
        espconn_sent(gConn, buffer, strlen(buffer));
        espconn_disconnect(gConn);
}
*/
